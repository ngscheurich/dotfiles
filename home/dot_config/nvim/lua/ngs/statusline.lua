...
local function get_mode_opts(mode)
  local mode_opts = {
    ...
    { modes = { "cv" }, name = "EX", color = colors.mode_ex, icon = "" },
    {
      modes = { "r", "rm", "r?", "!" },
      name = "...",
      color = colors.mode_wait,
      icon = "󰆅",
    },
    {
      modes = { "t" },
      name = "TERMINAL",
      color = colors.mode_terminal,
      icon = "",
    },
  }
  ...
  for _, opts in ipairs(mode_opts) do
    for _, m in ipairs(opts.modes) do
      if m == mode then return opts end
    end
  end
end

local function mode_init(self) self.mode = vim.fn.mode(1) end
...
local function git_diff_provider(self, type, symbol)
  ...
  if self.status[type] then count = self.status[type] end

  if count > 0 then return symbol .. count end
end
...
local function lsp_provider(self)
  ...
  for _, client in ipairs(vim.lsp.get_clients({ bufnr = 0 })) do
    ...
    for _, name in ipairs(self.hidden) do
      if name == client.name then hidden = true end
    end
    if not hidden then table.insert(clients, client.name) end
  end
...
local git = function()
  ...
  return {
    condition = function() return ok and conds.is_git_repo() end,
    ...
    {
      provider = function(self) return git_diff_provider(self, "added", "+") end,
      hl = { fg = colors.vcs_added, bg = colors.bg },
    },
    {
      provider = function(self) return git_diff_provider(self, "removed", "-") end,
      hl = { fg = colors.vcs_removed, bg = colors.bg },
    },
    {
      provider = function(self) return git_diff_provider(self, "changed", "~") end,
      hl = { fg = colors.vcs_changed, bg = colors.bg },
    },
  }
end
...
local diagnostics = {
  ...
  update = { "DiagnosticChanged", "BufEnter" },
  {
    provider = function(self)
      return self.errors > 0
        and string.format(" %s %i", self.icons.error, self.errors)
    end,
  ...
  },
  {
    provider = function(self)
      return self.warns > 0
        and string.format(" %s %i", self.icons.warn, self.warns)
    end,
  ...
  },
  {
    provider = function(self)
      return self.infos > 0
        and string.format(" %s %i", self.icons.info, self.infos)
    end,
  ...
  },
  {
    provider = function(self)
      return self.hints > 0
        and string.format(" %s %i", self.icons.hint, self.hints)
    end,
...
local filetype = function()
  ...
  if ok then
    return {
      condition = function() return ok end,
      init = function(self)
        local icon, hl_group =
