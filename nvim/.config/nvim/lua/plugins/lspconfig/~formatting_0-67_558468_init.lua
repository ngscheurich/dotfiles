...
-- Register snippet support for all servers
...
for _, server in ipairs(custom_servers) do
    require("plugins.lspconfig.servers." .. server)(config)
end

-- Format on save for servers that support it
-- local pat = table.concat({ "*.ex", "*.exs"}, ",")
-- local autocmd = string.format("BufWritePre %s lua vim.lsp.buf.formatting_sync()", pat)
-- ngs.augroup("lspformat", {autocmd})
